...
local AUTH_URL = os.getenv("FORWARD_AUTH_URL")
  or ("http://127.0.0.1:10080" .. "/outpost.goauthentik.io/auth/nginx")
...
local function forward_auth(txn)
  ...
  -- Never trust identity supplied by the client, including on error paths.
  ...
  if status == 401 then
    local sign_in = "/outpost.goauthentik.io/start?rd=" .. percent_encode(original_url)
    set_result(txn, "unauthorized", sign_in, set_cookie)
    return
  end
